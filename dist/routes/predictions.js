...
router.get('/mine', auth_1.authMiddleware, async (req, res) => {
    try {
        ...
        const result = await db_1.pool.query('SELECT match_id, predicted_home, predicted_away FROM predictions WHERE user_id = $1', [userId]);
...
router.post('/bulk', auth_1.authMiddleware, async (req, res) => {
    try {
        ...
        for (const p of predictions) {
            await db_1.pool.query(`INSERT INTO predictions (user_id, match_id, predicted_home, predicted_away)
         VALUES ($1, $2, $3, $4)
...
router.get('/points', auth_1.authMiddleware, async (req, res) => {
    try {
        const userId = req.user.id;
        const result = await db_1.pool.query(`SELECT p.match_id, p.predicted_home, p.predicted_away,
